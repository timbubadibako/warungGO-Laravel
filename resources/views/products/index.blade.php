            ...
            <div class="bg-white/70 backdrop-blur-sm rounded-2xl p-6 border border-gray-200/50 shadow-sm hover:shadow-md transition-all duration-200">
                <div class="flex items-center">
                    ...
                    <div class="ml-4">
                        ...
                        <p class="text-2xl font-bold text-gray-800">{{ $products->where('stock', '<=', 10)->count() }}</p>
                    </div>
                </div>
            </div>
            ...
            <div class="bg-white/70 backdrop-blur-sm rounded-2xl p-6 border border-gray-200/50 shadow-sm hover:shadow-md transition-all duration-200">
                <div class="flex items-center">
                    ...
                    <div class="ml-4">
                        ...
                        <p class="text-xl font-bold text-gray-800">Rp {{ number_format($products->sum(function($p) { return $p->stock * $p->purchase_price; }), 0, ',', '.') }}</p>
                    </div>
                </div>
            </div>
        </div>
                        ...
                        <tbody class="divide-y divide-gray-200/50">
                            @foreach($products as $product)
                                <tr class="hover:bg-blue-50/30 transition-colors duration-150">
                                    <td class="px-6 py-4">
                                        <div class="flex items-center">
                                            <div class="w-12 h-12 bg-gradient-to-br from-blue-100 to-green-100 rounded-xl flex items-center justify-center mr-3 overflow-hidden">
                                                <img
                                                    src="https://placehold.co/48x48/3b82f6/ffffff?text={{ substr($product->name, 0, 2) }}"
                                                    alt="{{ $product->name }}"
                                            ...
                                            <div>
                                                ...
                                                <p class="text-sm text-gray-500">{{ $product->barcode ?? 'N/A' }}</p>
                                            </div>
                                        </div>
                                    </td>
                                    <td class="px-6 py-4">
                                    ...
                                    <td class="px-6 py-4">
                                        <div class="space-y-1">
                                            <p class="text-sm text-gray-500">Beli: <span class="font-medium text-gray-700">Rp {{ number_format($product->purchase_price, 0, ',', '.') }}</span></p>
                                            <p class="text-sm font-semibold text-green-600">Jual: Rp {{ number_format($product->selling_price, 0, ',', '.') }}</p>
                                        </div>
                                    </td>
                                    <td class="px-6 py-4">
                                        <div class="flex items-center">
